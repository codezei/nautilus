...
function cloudFeaturesSlider() {
  var swiper = new Swiper(".cloud-features-slider", {
    slidesPerView: 1,
    pagination: {
...
function toggleTimeline() {
  ...
  var activeItem = document.querySelector('.timeline__item.active');
...
function teamSlider() {
  var popup = document.querySelector('.meet-team-popup');
  if (!popup) return;
  ...
  var employees = document.querySelectorAll('.cart-employee');
...
function customSelect() {
  ...
  for (var i = 0; i < select.length; i++) {
    var selected = select[i].querySelector('.selected');
    var input = select[i].querySelector('input');
    var current = select[i].querySelector('.option.current');
...
function changeRates() {
  var ratesSelect = document.querySelector('.rates__select');
  if (!ratesSelect) return;
  var fixedLine = document.querySelector('.js-rates-fixed');
  var mobile = document.querySelector('.js-rates-mobile');
  var value = ratesSelect.querySelector('.option.current').dataset.value.split(';');
  fixedLine.innerHTML = value[0];
  mobile.innerHTML = value[1];
  ratesSelect.addEventListener('click', function (e) {
    if (e.target.classList.contains('option')) {
      var _value = e.target.dataset.value.split(';');
      fixedLine.innerHTML = _value[0];
      mobile.innerHTML = _value[1];
    }
  });
}
}();
